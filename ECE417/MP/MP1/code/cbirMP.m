...
global file_name;
file_name=handles.filenames(imgInd,:);
...
num_match=length(find(feedback));
global query_counter;
global percision;

if(query_counter>0)
    percision=[percision,num_match/20];
    fprintf('Feedback provided %.0f times\n',query_counter);
...
q_c=sum(handles.META_DATA(:,handles.posInds),2)/length(handles.posInds);
...
[num_files,~]=size(handles.filenames);
D=[zeros(num_files,1),linspace(1,num_files,num_files)'];

for i=1:num_files
    D(i,1)=(q_c-handles.META_DATA(:,i))'*handles.W*(q_c-handles.META_DATA(:,i));
end

%D_temp=(q_c-handles.META_DATA(:,:))'*handles.W*(q_c-handles.META_DATA(:,:));
%diff=D_temp-D(:,1);
%unique(diff)

D=sortrows(D,[1,2]);
handles.currentTopInds=D(1:20,2)';
...
if(~isempty(handles.currentTopInds))
    %for i=1:47
     %   W(i,i)=1/(var(handles.META_DATA(i,handles.posInds))+0.0222);
    %end
    temp=1./(var(handles.META_DATA(:,handles.posInds),0,2)+0.0222);
    W=diag(temp);
