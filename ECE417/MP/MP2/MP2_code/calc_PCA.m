function [Y,principle_eigVal,Error_est,Error_true]=calc_PCA(input_data,energy)
...
while(sum_min<(1-energy)*total_energy)
    least_N=least_N+1;
    sum_min=sum_min+eigval(least_N);
end
principle_eigVal=eigval(end:-1:least_N);
principle_PCA=orth_PCA(:,end:-1:least_N);%reordered
Y=principle_PCA'*(X_tilde*sqrt(M-1));

X_hat=principle_PCA*Y+repmat(mean_wrt,[1,80]);
Error_est=sum(sum(abs(input_data-X_hat).^2,1))/(M-1);
Error_true=sum(eigval(1:least_N-1));
