...
X = abs(fft(frames,NFFT,1)); % X = [NFFT x Nf]. Each column is a spectrum with NFFT frequencies
X = X./NFFT;

% Apply Mel filterbank to spectra
%win=[diag(ones(1,K)),zeros(K,NFFT-K)];
win=eye(K,NFFT);

Y = H*win*X; % Y should be of size = [M x Nf]
