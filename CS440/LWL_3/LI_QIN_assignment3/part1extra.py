            ...
            for j in range(w):
                for di in range(i*hstride,i*hstride+n):
                    for dj in range(j*wstride,j*wstride+m):
                        if self.img[di][dj] == '+': 
                            self.val[i,j] = int(3*self.val[i,j]+1)
                        elif self.img[di][dj] == '#':
                            self.val[i,j] = int(3*self.val[i,j]+2) #applying tenary feature
                        else:
                            self.val[i,j] = int(3*self.val[i,j])
        return
...
if __name__ == '__main__':
    ...
    prob_map = np.zeros((10,int(3**(n*m)),height,width))     # (class,value,hidx,widx)
    for digit in trainset_sorted:
        if digit.lab==curlabel:
            ...
            prob_map[curlabel,:,:,:] = (prob_map[curlabel,:,:,:] + k)/(priors[curlabel] + (3**(n*m))*k)
            curlabel += 1
            priors[curlabel] += 1
            for i in range(height):
                for j in range(width):
                    prob_map[curlabel,int(digit.val[i,j]),i,j] += 1
    prob_map[9,:,:,:] = (prob_map[9,:,:,:] + k)/(priors[9] + (3**(n*m))*k)
