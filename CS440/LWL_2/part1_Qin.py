...
class flowfree:
	...
	def isconnected(self, start, goal,path,color):
		...
		for i in range(0, height):
			for j in range(0, width):
				if self.graph[i][j] != '_' :
					visited[i][j] = 1
		for i in path:
			if color ==self.graph[startx][starty]:
				visited[i[0]][i[1]] = 0
			else:
				visited[i[0]][i[1]] = 1
	...
	def change_boundary2(self,boundary,path):
		#print()
		for i in path:
			x=i[0]
			y=i[1]
			if x - 1 >= 0 and y - 1 >= 0: # set surrounding area to be boundary
				if boundary[x-1,y-1] == 0:
					boundary[x-1,y-1] = 1
			if x - 1 >= 0:
				if boundary[x-1,y] == 0:
					boundary[x-1,y] = 1
		...
		for i in range(4*self.width):
			...
			for path in Search_result:
				...
				if result != False:
					return result
				for j in path:
					self.graph[j[0]][j[1]] = '_'
				self.graph[self.color2pos[c][0][0]][self.color2pos[c][0][1]]=color
				self.graph[self.color2pos[c][1][0]][self.color2pos[c][1][1]]=color
		...
		for p in colorlist:
			for i in range(cutoff):
				...
				for path in Search_result:
					...
					for j in path:
						self.graph[j[0]][j[1]] = '_'
					self.graph[self.color2pos[color][0][0]][self.color2pos[color][0][1]]=color
					self.graph[self.color2pos[color][1][0]][self.color2pos[color][1][1]]=color
					self.colors.append(color)
